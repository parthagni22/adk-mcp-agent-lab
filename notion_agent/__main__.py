...
@click.option(
    "--port",
    "port",
    default=int(os.getenv("A2A_NOTION_PORT", 8004)),
    show_default=True,
    type=int,
...
def main(host: str, port: int) -> None:
    ...
    try:
        ...
        agent_executor = NotionADKAgentExecutor(agent=adk_agent, agent_card=agent_card)

    except Exception as e:
        logger.error(f"Failed to initialize Notion Agent components: {e}", exc_info=True)
        return

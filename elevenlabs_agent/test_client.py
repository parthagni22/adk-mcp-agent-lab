# elevenlabs_agent/test_client.py
import asyncio
import httpx
import os
import traceback
...
from a2a.types import (
    ...
    MessageSendParams,
    # Part, # Not directly used for type checking parts if assuming dicts
    SendMessageRequest,
    SendMessageResponse,
    SendMessageSuccessResponse,
    Task,
    TaskQueryParams,
    # TextPart, # Not directly used
)
...
async def run_elevenlabs_tts_test(client: A2AClient) -> None:
    ...
    if not extracted_task_id:
        ...
        print_json_response(agent_reply_data, "Agent's reply (send_response.result) for debugging")
        return
                                ...
                                if isinstance(parts_list, list):
                                    for j, part_data in enumerate(parts_list):
                                        ...
                                        if isinstance(part_data, dict):
                                            ...
                                            print(f"  Artifact {i}, Part {j} (unexpected item type): {part_data}")
                                else:
                                    print(f"  Artifact {i} (no 'parts' list or not a list): {artifact_item}")
                            else:
                                print(f"  Artifact {i} (unexpected type, not a dict): {artifact_item}")

                    elif task_status == "failed" and actual_task_result.status.message:
                        print(f"Task Failed Message: {actual_task_result.status.message}")
...
async def main() -> None:
    ...
    try:
        ...
        print(f'Connection error: Could not connect to agent at {AGENT_URL}. Ensure the server is running.')
        print(f'Details: {e}')
        traceback.print_exc()
    except Exception as e:

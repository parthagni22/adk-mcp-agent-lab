...
class RemoteConnections:
    ...
    async def invoke_agent(self, agent_url: str, query: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(send_response, 'root') and isinstance(send_response.root, SendMessageSuccessResponse):
            ...
            return {"error": "No immediate response content and could not extract taskId for polling"}
            
        except httpx.ConnectError:
            return {"error": f"Could not connect to agent at {agent_url}. Ensure the server is running."}
        except Exception as e:
            return {"error": f"Error calling agent at {agent_url}: {str(e)}"}

    def _create_send_message_payload(self, text: str, task_id: str | None = None, context_id: str | None = None) -> Dict[str, Any]:
    ...
    async def _poll_task_completion(self, client: A2AClient, task_id: str) -> Dict[str, Any]:
                ...
                if isinstance(get_response, GetTaskSuccessResponse) and get_response.result:
                    ...
                    if actual_task_result.status:
                        ...
                        if task_status == "completed":
                                            ...
                                            for part_data in parts_list:
                                                if isinstance(part_data, dict):
                                                    ...
                                                    if text:
                                                        results.append(text)
                                                    if audio_url:
                                                        results.append(f"Audio URL: {audio_url}")
                            ...
                            error_msg = actual_task_result.status.message if actual_task_result.status.message else "Task failed"

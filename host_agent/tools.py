...
from config import NOTION_AGENT_A2A_URL, ELEVENLABS_AGENT_A2A_URL
...
async def delegate_task(agent_name: str, task_description: str) -> str:
    ...
    remote_connections = await RemoteConnections.create(timeout=60.0)  # Increased timeout for complex tasks
    ...
    try:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(asyncio.run, delegate_task(agent_name, task_description))
                return future.result(timeout=90)  # Generous timeout for orchestrated tasks
        except RuntimeError:

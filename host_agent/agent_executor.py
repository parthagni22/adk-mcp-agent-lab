...
HOST_A2A_APP_NAME = "host_a2a_app"
...
class HostADKAgentExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            if not user_input:
                logger.warning(f"No user input found for {self.agent.name}; using default message.")
                user_input = "Please orchestrate a multi-agent workflow"
            
            logger.info(f"{self.agent.name} processing orchestration request: '{user_input}'")
            
            # Use a consistent user_id for Host agent
            ...
            logger.info(f"Using session_id: {session_id} (from context_id: {context.context_id})")
            ...
            if not adk_session:
                ...
                logger.info(f"Retrieved existing ADK session: {session_id} for {self.agent.name}")
            ...
            logger.error(f"Error executing orchestration in {self.agent.name}: {str(e)}", exc_info=True)
            error_message_text = f"Error in orchestration workflow: {str(e)}"
            event_queue.enqueue_event(
                new_agent_text_message(
    ...
    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        logger.info(f"Cancelling Host orchestration task: {task_id} for agent {self.agent.name}")
        
        timestamp = datetime.datetime.now(datetime.timezone.utc).isoformat()
        
        # The correct attribute is lowercase `canceled` as per the a2a.types.TaskState Enum
